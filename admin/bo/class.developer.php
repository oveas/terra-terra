...
/**
 * \file
 * This file defines the base for webbased OWL development
...
class Developer extends _OWL
{
	...
	/**
	 * Generate an OWL application using the data taken from the form
	 * \return True on success
	 * \author Oscar van Eijk, Oveas Functionality Provider
	 */
	public function generateApplic ()
	{
		if (!OWLloader::getClass('dirhandler')) {
		...
		$this->tempDir = OWL_TEMP . '/owl-' . strtolower($this->appCode);
	...
	{
		return "$comment *****
$comment This file is generated by the OWL Developers class.
$comment
";
	}
...
";
	...
	filename = OWL_LOG/" .strtolower($this->appCode) . ".log
	log_level = OWL_DEBUG
...
";
	...
	private function createInstaller ()
	{
		$_fileContent = "<?php
" . $this->createFileHeader() . "
define ('OWL_ROOT', '" . OWL_ROOT . "');	// OWL Root *must* be defined here.
require (OWL_ROOT . '/OWLinstaller.php');	// Load the OWL Installer class

// This creates the application in the OWL database
//
\$_id = OWLinstaller::installApplication(
	  '$this->appCode'		// Application code
	, '/owl-" . strtolower($this->appCode) . "'		// Relative URL to the application
	, '$this->appName'		// Application name
	, 'A1.0'		// Version numner
	, '$this->appDesc'		// Description
	, 'http://localhost/owl-" . strtolower($this->appCode) . "'		// URL to the application developer's site
...
OWLinstaller::addConfig(\$_id, 'database', 'prefix', '$this->appCode', true);
OWLinstaller::addConfig(\$_id, 'locale', 'lang', 'nl-NL');
...
//
//OWLinstaller::installTables('" . OWL_SITE_TOP . "/owl-" . strtolower($this->appCode) . "/admin/$this->appCode.sql', false);
