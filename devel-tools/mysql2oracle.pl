...
{
	my ($tbl, $col) = @_;
	my $trg = $tbl . $col . $main::suffixTrigger;
	my $seq = $tbl . $col . $main::suffixSequence;

	if (length($trg) > 30) {
		my $sub = (30 - length($main::suffixTrigger)) / 2;
		$trg = substr ($tbl, 0, $sub) . substr ($col, 0, $sub) . $main::suffixTrigger;
	}
	if (length($seq) > 30) {
		my $sub = (30 - length($main::suffixSequence)) / 2;
		$seq = substr ($tbl, 0, $sub) . substr ($col, 0, $sub) . $main::suffixSequence;
	}
...
sub addConstraint($$$$)
{
	...
	my $cst = $tbl . $col . $main::suffixConstraint;

	if (length($cst) > 30) {
		my $sub = (30 - length($main::suffixConstraint)) / 2;
		$cst = substr ($tbl, 0, $sub) . substr ($col, 0, $sub) . $main::suffixConstraint;
	}
	$$mSize = 0;
...
sub addIndex($$$$)
{
	...
	return 'CREATE ' . ($unique ? 'UNIQUE ' : '') .  "INDEX $main::oracleQuotes$main::idxPrefix$idx$main::oracleQuotes \n"
...
while (my $line = <>) {
	...
	# Commit statements can be just copied
	if ($line =~ /^\s*DROP/) {
		print $line."\n";
		next;
	}
	
	# In INSERT statements, remove or replace the backticks and copy the line.
	# This will only work when the complete INSERT statement is on 1 line, 1 insert per line,
	# as it is generated by MySQL workbench.
	if ($line =~ /^\s*INSERT\s+INTO/) {
		$line =~ s/(INSERT\s+INTO)\s+(`)?(\w+)(`)?/$1 $main::oracleQuotes$3$main::oracleQuotes/g;
	...
	if ($line =~ s/(CREATE\s+TABLE(.*?)(\`$main::mysqlScheme\`\.)?\`(\w+)\`\s*\()/CREATE TABLE $main::oracleQuotes$4$main::oracleQuotes ( /) {
		$tableName = $4;
	}
	...
	# If there's a 'CREATE INDEX' line, remove unsupported elements, output and get the next
	# If it's a foreign key, skip it completely (it's set as a constraint already)
	if ($line =~ /^CREATE\s+(UNIQUE\s+)?INDEX/) {
		if ($line =~ /`(fk_\w+?)`/) {
			next;
		}
		$line =~ s/USING\s+[A-Z]+//;
		$line =~ s/`(\w+?)`/$main::oracleQuotes$main::idxPrefix$1$main::oracleQuotes/;
		$line =~ s/`/$main::oracleQuotes/g;
		print $line."\n";
		next;
	}
